...
- [ ] Tune the baseline (regularization strength) and evaluate a tree-based ensemble (e.g., Random Forest or XGBoost); compare on PR-AUC
- [ ] Select a decision threshold on validation data and complete error analysis before candidate selection
- [ ] Run clustering as exploratory unsupervised analysis
- [ ] Run PCA as exploratory unsupervised analysis
- [ ] Convert supervised model training into a callable Python script (`main()`, not notebook-only)
...
## Status
🚧 In progress — Phase 1 build underway. EDA and feature engineering are complete with submitted reports; the baseline logistic regression model is trained and validated. Next up: model comparison against a tree-based ensemble, the clustering and PCA analyses, and the MLflow/FastAPI serving layer.
## Author
Angeline Setiawan
